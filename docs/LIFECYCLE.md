...
| Scope — custom excludes, no flags | folder anchored with `--exclude *.tmp`; verifier given no flags | exit 0; "from the manifest's scope block" | `test_custom_exclude_folder_verifies_with_no_flags_via_manifest_scope` |
| Scope — explicit flag wins | wrong `--exclude` passed | exit 3; "overrides manifest scope" | `test_explicit_exclude_overrides_manifest_scope` |
...
| Scope — absent | manifest without `scope` | exit 0; "standard deny-list" | `test_manifest_without_scope_uses_standard_denylist` |
