...
| Filename / label | yes | opt-in | no | Off by default; pass `--label` (CLI) or the `label` argument (MCP tools) to include. The web page sends a fixed source tag (`v2-homepage`), never the file name |
