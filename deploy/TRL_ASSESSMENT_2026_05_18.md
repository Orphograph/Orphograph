...
| 19 | Cloudflare DNS | **8** | MEMORY confirms Cloudflare DNS in front of Fly; force-https in `fly.toml`; SPF/DKIM/DMARC live per the email-infrastructure memory note. | DNSSEC enabled and verified; CAA record locks issuance to a single CA. | 0.25h DNSSEC + CAA |
