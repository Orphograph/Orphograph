...
**What fails.** ~~All Bitcoin revenue lands at the address in `data/btc_address.txt`.~~ NO LONGER TRUE as of 2026-09-19: the direct on-chain BTC rail was retired, nothing reads that file, and no revenue lands at any address of ours. The paragraph is kept as the record of a real gap that existed on 2026-05-18, with its present-tense claim and the address literal struck — a dated audit may say what WAS true, but it must not go on asserting a 'current value' that is not. Historically: recovery of that address depended entirely on the BIP39 seed phrase held by the founder. Single-location seed loss modes: house fire, theft, flood, founder-incapacitation with no heir access. Server compromise is **not** in scope here — the seed never touches the Fly host; the host only holds the receive address. Detection is post-hoc: founder tries to sweep funds and the wallet won't restore. There is no remote alarm for "your steel plate is missing."
...
4. ~~Document the recovery runbook in `deploy/WALLET_QUICK.md`~~ — MOOT as of 2026-09-19: the direct on-chain BTC rail was retired and that runbook was deleted with it. There is no hot wallet to recover. (This line is left struck rather than removed: the file is a dated audit record, and a dangling pointer to a deleted runbook is a broken instruction.)
