...
- [x] **#10 MCP tool** — `orphograph_anchor_folder(path)` added to `mcp/orphograph_mcp.py`: walks a folder, hashes locally into the RFC-6962 Merkle tree (stdlib replica of server/merkle.py), POSTs only the manifest to /api/anchor_folder, returns receipt + `/certificate/<id>`. Verified the MCP root is **byte-identical** to the server's `from_folder` (same root + leaf order) and accepted by the live endpoint. Tests lock cross-compatibility (no network). Also **re-synced the stale download mirror** `web/mcp/orphograph_mcp.py` to the canonical (it predated `anchor_output`). _(commit pending)_
- [x] **#11 `paths_public` producer side** — `/api/anchor_folder` now accepts `paths_public` in the request, stores it on the receipt, and echoes it; `provenance.py anchor --public-paths` sends it. Verified end-to-end: a `--public-paths` anchor renders the full manifest to non-owners, a default anchor stays redacted. _(Folder-anchor UI checkbox split to #23, pending the #24 investigation.)_ _(commit pending)_
- [x] **#24 FIXED a real bug: the web folder-anchor never worked** — confirmed `web/folder.js` POSTed a `{files:[...]}` summary that `/api/anchor_folder` rejects with 400 ("manifest leaves must be a non-empty list"); anchoring a folder from the website always failed. Fixed folder.js to emit the server-format `{algorithm, version:1, root_hex, leaves:[{path,file_sha256_hex,leaf_hex,size_bytes}]}` manifest (and capture `size_bytes` during hashing); bumped `folder.js?v=2`. Proved folder.js's crypto is correct (a verbatim node replica yields the canonical root a51e1409…) — only the wire format was wrong. Regression guards added (source-level + server contract). _(commit pending)_
...
- [ ] **#21 MCP `orphograph_verify_folder(receipt_id, path)`** — verify a local bundle against a live folder receipt through MCP (the agent-facing twin of `provenance.py verify --receipt`): fetch the root, rebuild locally, report match/mismatch. Optionally a per-file inclusion tool.
- [ ] **#23 Folder-anchor UI "publish paths" checkbox** — add an opt-in checkbox to the homepage folder-anchor block that sends `paths_public` on submit. UNBLOCKED — #24 fixed folder.js to post a valid `{leaves}` manifest; the checkbox now just adds `paths_public` to the wrapped body.
- [ ] **#22 Refresh the MCP package build** — `mcp/dist/*.whl|tar.gz` predate the new tool; regenerate so `pip install` ships `anchor_folder` (build-step, low urgency).
