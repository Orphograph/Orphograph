...
- [ ] **#24 INVESTIGATE the web folder-anchor path (possible latent bug)** — `web/folder.js` `_submitManifest()` POSTs a manifest shaped `{algorithm, root_hex, file_count, files:[{path,file_sha256}]}`, but `/api/anchor_folder` requires `leaves:[{path,file_sha256_hex,leaf_hex,size_bytes}]` (via `merkle.from_manifest`). Confirm whether anchoring a folder from the website actually works end-to-end (curl the real flow / read more of folder.js for a transform I may have missed). If it's broken, fix folder.js to emit the server-format `leaves` manifest. Surfaced while wiring #11.
...
- [ ] **#23 Folder-anchor UI "publish paths" checkbox** — add an opt-in checkbox to the homepage folder-anchor block that sends `paths_public` on submit. BLOCKED on #24 (the web folder-anchor POST shape needs confirming/fixing first).
